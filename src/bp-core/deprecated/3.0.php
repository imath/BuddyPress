	...
	if ( bp_is_current_component( 'admin' ) && bp_is_current_action( 'delete-user' ) ) {
		...
		if ( bp_core_delete_account( bp_displayed_user_id() ) ) {
			bp_core_add_message(
				...
				sprintf( __( '%s has been deleted from the system.', 'deprecated string', 'buddypress' ), bp_get_displayed_user_fullname() )
			);
		} else {
			bp_core_add_message(
				/* translators: %s: member name */
				sprintf( __( 'There was an error deleting %s from the system. Please try again.', 'deprecated string', 'buddypress' ), bp_get_displayed_user_fullname() ),
				'error'
			);
